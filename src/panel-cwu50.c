...
static void cwu50_init_sequence(struct cwu50 *ctx)
{
	...
	dev_info(ctx->dev, "cwu_50_init_sequence begin");
	...
	dev_info(ctx->dev, "cwu_50_init_sequence finished");
}
...
static int cwu50_disable(struct drm_panel *panel)
{
	...
	dev_info(ctx->dev, "cwu_50_disable begin");

	// dump_stack();

	if (!ctx->enabled)
	{
		dev_info(ctx->dev, "cwu_50_disable exit because ctx is not enabled ");
		return 0;
	}
		

	dev_info(ctx->dev, "cwu_50_disable disabling backlight");
	backlight_disable(ctx->backlight);

	ctx->enabled = false;

	dev_info(ctx->dev, "cwu_50_disable marking ctx->enabled to false");
...
static int cwu50_unprepare(struct drm_panel *panel)
{
	...
	dev_info(ctx->dev, "cwu_50_unprepare begin");
...
static int cwu50_prepare(struct drm_panel *panel)
{
	...
	dev_info(ctx->dev, "cwu50_prepare begin");

	if (ctx->prepared)
	{
		dev_info(ctx->dev, "cwu50_prepare preliminary exit because of ctx is prepared already");
		return 0;
	}
	...
	dev_info(ctx->dev, "cwu50_prepare finish");
...
static int cwu50_enable(struct drm_panel *panel)
{
	...
	dev_info(ctx->dev, "cwu50_enable begin");

	if (ctx->enabled)
		return 0;

	dev_info(ctx->dev, "cwu50_enable backlight enable");
...
static int cwu50_get_modes(struct drm_panel *panel, struct drm_connector *connector)
{
	...
	dev_info(ctx->dev, "cwu50_get_modes");
...
static int cwu50_probe(struct mipi_dsi_device *dsi)
{
	...
	dev_info(dev, "cwu50_probe begin");
